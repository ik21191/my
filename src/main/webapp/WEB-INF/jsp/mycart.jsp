  ...
  <tr>
    <td align="center" valign="top">
        ...
        <div class="thanku_row_inn">
            <img src="${p.productSmallImage}" alt="flower" />
            <p>${p.productName}</p>
        </div>
        </c:forEach>
        <c:forEach items="${addonList}" var="ad">
        <input type="hidden" name="addonsid" value="${ad.addonId}"/>
        <div class="thanku_row_inn">
            <img src='${ad.addonSmallImage}' alt="flower" />
            <p>${ad.addonName}</p>
        </div>
        </c:forEach>
    </td>
    <td align="center" valign="top"><c:forEach items="${list}" var="p">
    ...
    </div></c:forEach><c:forEach items="${addonList}" var="ad">
    <div class="than_qty_amt">
    ...
    <c:forEach items="${list}" var="p">
        ...
        <input id="pp" type="hidden" value="${p.productPrice}"/>${p.productPrice}</div>
    </c:forEach>
    <c:forEach items="${addonList}" var="ad">
    <div id="addonPrice" class="than_qty_amt">
    <input name="ap" type="hidden" value="${ad.addonPrice}"/>${ad.addonPrice}</div> 
    </c:forEach>   
    </td>
...
<div class="footer_row">
    ...
    <li><a href="http://www.fnp.com/flowers/faces/jsp/serviceablecities.jsp" title="send flowers anywhere in India">View all Cities &gt;&gt;</a></li>
        ...
        <div class="footer_colum footer_display5">
                  <h4>International Delivery</h4> <ul>
